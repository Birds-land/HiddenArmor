...
public class HiddenArmorCommand {
	...
	public HiddenArmorCommand(HiddenArmor plugin){
		...
		new CommandUtil(plugin, "hiddenarmor", 0, 2, false, true){
			...
			private boolean toggleArmor(CommandSender sender, String[] arguments) {
				...
				final String action = arguments[0].toLowerCase();
	...
	private void help(CommandSender sender){
		...
		final boolean togglePermission = plugin.getConfig().getBoolean("default-permissions.toggle");
		final boolean toggleOtherPermission = plugin.getConfig().getBoolean("default-permissions.toggle-other");
